...
from statmatch.knn_hotdeck import KNNHotDeckClassifier
...
def build_sgdClassifiers(random_state=1980):
    param_grid = {
        'loss': ['log'],                                            # logistic regression
        'penalty': ['elasticnet', 'l1', 'l2'],
        'l1_ratio': [0, 0.1, 0.5, 0.7, 1.0],                        # l1_ratio is alpha in H2O
        'alpha' : [0.000819600824638717, 0.26221219843782834]       # alpha is lambda in H2O
    }

    return build_models(SGDClassifier, param_grid)


def build_randomForestClassifiers(random_state=1980):
    param_grid = {
        'n_estimators': [50, 100],
        'criterion':  ['gini', 'entropy'],                # h2o uses residual sum of the squares (rss)
        'max_depth': [20],        
        'max_features': ['sqrt'],                         # h2o uses square root
        'min_samples_split' : [1e-4, 1e-5]
#       'sample_rate' : [1],                              # no parallel in scikit
#       'col_sample_rate_per_tree' : [.5],                # no parallel in scikit
    }

    return build_models(RandomForestClassifier, param_grid)


def build_gradientBoostingClassifiers(random_state=1980):
    param_grid = {
        'n_estimators': [5, 10],                          # ntrees in h2o
        'max_depth': [20],
        'learning_rate' : [0.5, 1.],                      # learn_rate in h2o
        'subsample': [0.6320000291, 1.],                  # sample_rate in h2o
        'min_samples_split' : [1e-4, 1e-5]                # min_split_improvement       
#       'col_sample_rate_per_tree' : [.7],                # no parallel in scikit
    }

    return build_models(GradientBoostingClassifier, param_grid)


def build_deepLearningClassifiers(random_state=1980):
    param_grid = {
        'activation': ['tanh', 'relu', 'logistic'],
        'hidden_layer_sizes' : [(10, 10), (10, 10, 10)], 
        'max_iter' : [5, 10],                             # epochs in h2o
        'alpha' : [.0, .5]                                # l2 in h2o
#        'input_dropout_ratio' : [.1],                    # no parallel in scikit
#        'l1' : [.0],                                     # no parallel in scikit
    }
...
def build_knnHotDeckClassifiers(random_state=None):
    param_grid = {
        'distance': ['cityblock', 'braycurtis'],
	'neighbors' : [2, 30]
    }

    return build_models(KNNHotDeckClassifier, param_grid)
...
#    'lr': build_logisticRegressionClassifiers,
    'dl': build_deepLearningClassifiers,
    'sm': build_knnHotDeckClassifiers 
}
